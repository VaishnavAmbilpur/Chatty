...
import { Users, Info, Smiley, PaperPlaneRight, SignOut } from '@phosphor-icons/react'
    ...
    <div className='flex flex-col md:flex-row gap-4 md:gap-x-4 max-w-4xl w-full h-auto md:h-[550px] items-center justify-center p-4 md:p-0'>
      ...
      <div className='font-Josefin glass-chat animate-fade-in-up shadow-2xl flex-1 w-full max-w-[460px] h-[550px] md:h-full overflow-hidden rounded-xl text-white flex flex-col relative'>
        <div className='text-2xl tracking-tighter py-4 px-5 font-semibold flex items-start flex-wrap gap-x-3 gap-y-1.5 flex-col bg-white/5'>
          ...
          <div className='text-[10px] text-zinc-500 tracking-widest uppercase font-bold mt-0.5'>Room • {roomUsers.length} Online</div>
        </div>
          ...
          {loading ? (
            ...
            <div ref={mesRef} className='flex-1 flex flex-col gap-y-3 overflow-y-auto scrollbar-hide pr-2'>
              {messages.map((m, idx) => {
                ...
                return (
                  <div key={idx} className={`flex gap-x-2.5 animate-scale-in ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    <div className={`max-w-[85%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`py-2 px-3.5 rounded-xl text-sm ${isMe
                      ...
                      <span className='text-[8px] text-zinc-650 font-medium mt-1 px-1'>{new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div className='p-4 pt-1.5 bg-white/5 relative'>
          {showEmojis && (
            <div className='absolute bottom-full left-4 mb-2 bg-zinc-900 border border-white/10 p-1.5 rounded-lg flex gap-x-1.5 animate-scale-in flex-wrap max-w-[180px] shadow-2xl z-50'>
              {emojis.map(e => (
                <button key={e} onClick={() => addEmoji(e)} className='hover:bg-white/10 p-1 rounded transition-all text-base'>
                  {e}
                </button>
              ))}
            </div>
        ...
        <div className='w-full md:w-56 glass-chat h-auto md:h-full rounded-xl p-4 flex flex-col animate-scale-in'>
          ...
          <div className='flex-1 overflow-y-auto space-y-3 pr-2'>
            {roomUsers.map((user, i) => (
              <div key={i} className='flex items-center gap-x-2.5 group animate-fade-in-up' style={{ animationDelay: `${i * 100}ms` }}>
                <div className='relative'>
                ...
                <div className='flex flex-col'>
                  ...
                  <span className='text-[9px] text-zinc-500 font-bold uppercase'>{user.isAdmin ? 'Owner' : 'Member'}</span>
                </div>
              </div>
            ))}
          </div>
